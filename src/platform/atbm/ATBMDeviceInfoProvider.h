...
class ATBMDeviceInfoProvider : public DeviceInfoProvider
{
public:
    ATBMDeviceInfoProvider() = default;
    ~ATBMDeviceInfoProvider() override {}
    ...
    static ATBMDeviceInfoProvider & GetDefaultInstance();
...
protected:
    ...
    {
    public:
        UserLabelIteratorImpl(ATBMDeviceInfoProvider & provider, EndpointId endpoint);
        size_t Count() override { return mTotal; }
        bool Next(UserLabelType & output) override;
        void Release() override { chip::Platform::Delete(this); }

    private:
        ATBMDeviceInfoProvider & mProvider;
        EndpointId mEndpoint = 0;
        size_t mIndex        = 0;
        size_t mTotal        = 0;
