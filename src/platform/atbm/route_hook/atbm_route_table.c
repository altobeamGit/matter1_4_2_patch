...
static atbm_route_entry_t s_route_entries[MAX_RIO_ROUTE];

static atbm_route_entry_t * find_route_entry(const atbm_route_entry_t * route_entry)
{
...
static atbm_route_entry_t * find_empty_route_entry(void)
{
    for (size_t i = 0; i < LWIP_ARRAYSIZE(s_route_entries); i++)
    {
        if (s_route_entries[i].netif == NULL)
...
{
    atbm_route_entry_t * route = (atbm_route_entry_t *) arg;

    atbm_route_table_remove_route_entry(route);
}

atbm_route_entry_t * atbm_route_table_add_route_entry(const atbm_route_entry_t * route_entry)
{
    ...
    atbm_route_entry_t * entry = find_route_entry(route_entry);
...
int8_t atbm_route_table_remove_route_entry(atbm_route_entry_t * route_entry)
{
    ...
    for (atbm_route_entry_t * moved = route_entry; moved < &s_route_entries[LWIP_ARRAYSIZE(s_route_entries) - 1]; moved++)
    {
        *moved = *(moved + 1);
        if (moved->netif == NULL)
        {
...
static inline bool is_better_route(const atbm_route_entry_t * lhs, const atbm_route_entry_t * rhs)
{
    if (rhs == NULL)
    {
        return true;
...
static inline bool route_match(const atbm_route_entry_t * route, const ip6_addr_t * dest)
{
