...
typedef struct atbm_route_hook_t
{
    struct netif * netif;
    struct raw_pcb * pcb;
    struct atbm_route_hook_t * next;
} atbm_route_hook_t;
...
static atbm_route_hook_t * s_hooks;
...
static void ra_recv_handler(struct netif * netif, const uint8_t * icmp_payload, uint16_t payload_len, const ip6_addr_t * src_addr)
{
    ...
    while (payload_len >= 2)
    {
            ...
            payload_len >= opt_len)
        {
            ...
            iot_printf("Received RIO\n");
            if (rio_data_len >= prefix_len_bytes)
            {
                ip6_addr_t prefix;
                atbm_route_entry_t route;
                ...
                iot_printf("prefix %s lifetime %u\n", ip6addr_ntoa(&prefix), route.lifetime_seconds);
                if (atbm_route_table_add_route_entry(&route) == NULL)
                {
                    iot_printf("Failed to add route table entry\n");
                }
            }
        }
...
static uint8_t icmp6_raw_recv_handler(void * arg, struct raw_pcb * pcb, struct pbuf * p, const ip_addr_t * addr)
{
    ...
    ip6_addr_t src;
    ip6_addr_t dest;
    atbm_route_hook_t * hook = (atbm_route_hook_t *) arg;
    ...
    if (p->tot_len != p->len)
    {
        iot_printf("Ignore segmented ICMP packet\n");
        return 0;
    }
    if (p->tot_len <= sizeof(struct ip6_hdr) + sizeof(struct icmp6_hdr))
    {
        iot_printf("Ignore invalid ICMP packet\n");
        return 0;
    }
...
int8_t atbm_route_hook_init(void)
{
    struct netif * lwip_netif = atbm_wifi_get_sta_netif();
    ip_addr_t router_group  = IPADDR6_INIT_HOST(0xFF020000, 0, 0, 0x02);
    atbm_route_hook_t * hook = NULL;
    int8_t ret              = 0;

    if (lwip_netif == NULL)
    {
        iot_printf("Invalid network interface\n");
        return -1;
    }

    for (atbm_route_hook_t * iter = s_hooks; iter != NULL; iter++)
    {
        if (iter->netif == lwip_netif)
        {
            iot_printf("Hook already installed on netif, skip...\n");
            return 0;
        }
    }

    hook = (atbm_route_hook_t *) malloc(sizeof(atbm_route_hook_t));
    if (hook == NULL)
    {
        iot_printf("Cannot allocate hook\n");
        return -1;
    }

    if(mld6_joingroup_netif(lwip_netif, ip_2_ip6(&router_group)) != ERR_OK)
    {
        iot_printf("Failed to join multicast group\n");
        ret = -1;
        goto exit;
    }
